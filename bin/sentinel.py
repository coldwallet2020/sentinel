...
def attempt_superblock_creation(billiecoind):
    ...
    maxgovobjdatasize = billiecoind.govinfo['maxgovobjdatasize']
    sb = billiecoinlib.create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize)
    if not sb:
...
def main():
    ...
    if not is_billiecoind_port_open(billiecoind):
        print("Cannot connect to billiecoind. Please ensure billiecoind is running and the JSONRPC port is open to Sentinel.")
        return

    # check billiecoind sync
    if not billiecoind.is_synced():
        print("billiecoind not synced with network! Awaiting full sync before running Sentinel.")
        return
